...
final class HomeVC: BaseVC, UITableViewDataSource, UITableViewDelegate,ThemedNavigation, SeedReminderViewDelegate {
    ...
    private lazy var tableView: UITableView = {
        ...
        result.dealCorner(type: .topLeftRight, corner: 20.w)
        result.separatorStyle = .none
        result.themeBackgroundColor = .conversationButton_background
...
//        createNewPrivateChatButton.set(.width, to: Values.iPadButtonWidth)
        ...
        let result = UIStackView(arrangedSubviews: [ explanationLabel])
...
//        newConversationButton.pin(.bottom, to: .bottom, of: view.safeAreaLayoutGuide, withInset: -Values.smallSpacing)
    ...
    private func updateNavBarButtons() {
        ...
        let labTitle = UILabel(font: UIFont.Bold(size: 20),textColor: .textPrimary,text: "EthMessenger")
        ...
        let addItem = UIBarButtonItem(image: UIImage(named: "icon_chats_add"), style: .done, target: self, action: #selector(createNewConversation))
        let searchItem = UIBarButtonItem(image: UIImage(named: "icon_chats_search"), style: .done, target: self, action: #selector(showSearchUI))
        searchItem.imageInsets = UIEdgeInsets(top: 0, leading: 10.w, bottom: 0, trailing: -10.w)
