...
class EMSelectTokenVC: EMAlertController {
    ...
    lazy var dataArr : [EMTokenModel] = {
        guard let network = EMNetworkModel.getNetwork() else{
            return []
        }
        let tokens = EMTableToken.selectTokenWithChainId(network.chain_id)
        return tokens
    }()
...
extension EMSelectTokenVC: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.token = self.dataArr[indexPath.row]
