...
public protocol SGPagingTitleViewDelegate{
    /// 获取当前选中标题下标值
...
public class SGPagingTitleView: UIView {
    ...
    public var delegate: SGPagingTitleViewDelegate?
...
public extension SGPagingTitleView {
    ...
    func resetTitle(color: ThemeValue, selectedColor: ThemeValue) {
    ...
    func setBackgroundImage(name: String, selectedName: String?, index: Int) {
...
private extension SGPagingTitleView {
    ...
    /// 重置标题颜色(color：普通状态下标题颜色、selectedColor：选中状态下标题颜色)
    func p_resetTitle(color: ThemeValue, selectedColor: ThemeValue) {
        for (_, btn) in tempBtns.enumerated() {
            btn.setThemeTitleColor(color, for: .normal)
            btn.setThemeTitleColor(selectedColor, for: .selected)
        }
...
private extension SGPagingTitleView {
    ...
    func addTitleBtns() {
        ...
        for idx in 0..<titleCount {
            ...
            btn.setThemeTitleColor(configure.color, for: .normal)
            btn.setThemeTitleColor(configure.selectedColor, for: .selected)
...
private extension SGPagingTitleView {
    ...
    func changeSelectedBtn(btn: UIButton) {
        ...
        if selectedFontName == defaultFontName && selectedFontPointSize == defaultFontPointSize {
            ...
            if configure.gradientEffect {
                tempBtns.forEach { (EBtn) in
                    EBtn.titleLabel?.themeTextColor = configure.color
                }
                btn.titleLabel?.themeTextColor = configure.selectedColor
            }
        } else {
            // 此处作用：避免滚动过程中点击标题手指不离开屏幕的前提下再次滚动造成的误差（由于文字渐变效果导致未选中标题的不准确处理）
            if configure.gradientEffect {
...
private extension SGPagingTitleView {
    ...
    /// 开始颜色
    func P_startColor(color: ThemeValue) {
        let components = P_getRGBComponents(color: ThemeManager.currentTheme.color(for: color) ?? UIColor.clear)
        startR = components[0]
        startG = components[1]
        startB = components[2]
    }
    /// 结束颜色
    func P_endColor(color: ThemeValue) {
        let components = P_getRGBComponents(color: ThemeManager.currentTheme.color(for: color) ?? UIColor.clear)
        endR = components[0]
        endG = components[1]
        endB = components[2]
