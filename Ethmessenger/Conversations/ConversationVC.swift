...
final class ConversationVC: BaseVC, ConversationSearchControllerDelegate,EMHideNavigationBarProtocol, UITableViewDataSource, UITableViewDelegate {
    ...
    lazy var tableView: InsetLockableTableView = {
        ...
        result.dealLayer(corner: 20.w)
        result.separatorStyle = .none
        result.themeBackgroundColor = .chatBackgroundPrimary
...
//        titleView.initialSetup(with: self.viewModel.initialThreadVariant)
        ...
        scrollButtonBottomConstraint = scrollButton.pin(.bottom, to: .bottom, of: view, withInset: -46)
    ...
    private func handleThreadUpdates(_ updatedThreadData: SessionThreadViewModel, initialLoad: Bool = false) {
        ...
        {
            self.navView.model = updatedThreadData
        }
    ...
    func updateNavBarButtons(threadData: SessionThreadViewModel?, initialVariant: SessionThread.Variant) {
        guard
            ...
            self.navView.btnCall.isHidden = true
            self.navView.btnMore.isHidden = true
            return
        }
        switch threadData.threadVariant {
            case .contact:
                self.navView.btnMore.isHidden = false
                if SessionCall.isEnabled && !threadData.threadIsNoteToSelf {
                    self.navView.btnCall.isHidden = false
                }
                else {
                    self.navView.btnCall.isHidden = true
                }
    ...
    @objc func handleKeyboardWillChangeFrameNotification(_ notification: Notification) {
        ...
        let changes = { [weak self] in
            self?.scrollButtonBottomConstraint?.constant = -(keyboardTop + 46)
